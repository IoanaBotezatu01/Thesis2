...
const modelPath = process.env.MODEL_DISEASES_PATH;
...
async function predictDiseaseClass(imagePath)
 {
    if (!model) {
...
async function handleDiseasePhotoPrediction(req, res)
 {
    try {
        const imagePath = req.file.path;
        const result = await predictDiseaseClass(imagePath);
        console.log(result);
...
module.exports = {
    handleDiseasePhotoPrediction: handleDiseasePhotoPrediction,
    predictDiseaseClass: predictDiseaseClass
};
